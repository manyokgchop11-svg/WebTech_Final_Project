...
4. Use your assigned database (for school servers) or create quickbite_db (for local XAMPP)
5. Import the database structure from database/setup.sql

# 3. Configuration
1. Clone/download project files to htdocs/quickbite
2. The system automatically detects your environment (local XAMPP vs school server)
3. Database configuration is handled automatically in config/database.php
4. Sample data is included in the database setup

#  4. Access the System
- **Website**: http://localhost/quickbite/index.php (main entry point)
- **Admin Login**: http://localhost/quickbite/auth/login_fixed.php → admin@quickbite.com / admin123 (select "Administrator" role)
- **Customer Login**: http://localhost/quickbite/auth/login_fixed.php → customer@test.com / customer123 (select "Customer" role)
...
#  Support
For technical support or questions, please contact manyokgchop11@gmail.com