 ...
 * 
 * This file automatically detects whether you're running on:
 * - School Server (webtech_2025A_manyok_deng database)
 * - Local XAMPP (quickbite_db database)
 * 
 * QUICK SETUP:
 * 1. For School Server: Just upload files and import database/setup.sql
 * 2. For Local XAMPP: Create 'quickbite_db' database and import database/setup.sql
 * 
 * MANUAL OVERRIDE:
 * Set $manual_override = true (school) or false (local) to force environment
 */

// Database configuration - Auto-detects environment (School Server vs Local XAMPP)
// Manual override: Set to true for school server, false for local XAMPP, or null for auto-detection
$manual_override = null; // Change to true/false to force environment
...
if ($is_school_server) {
    ...
    // Update these settings for your school server
    define('DB_HOST', 'localhost');
    define('DB_USER', 'manyok.deng');           // Your school username
    define('DB_PASS', 'M@ny0k');                // Your school password
    define('DB_NAME', 'webtech_2025A_manyok_deng'); // Your assigned database name
    define('DB_PORT', 3306);                    // Standard MySQL port
} else {
    // ===== LOCAL XAMPP CONFIGURATION =====
    // Default XAMPP settings - works out of the box
    define('DB_HOST', 'localhost');
    define('DB_USER', 'root');                  // Default XAMPP user
    define('DB_PASS', '');                      // Default XAMPP password (empty)
    define('DB_NAME', 'quickbite_db');          // Local database name
    define('DB_PORT', 3307);                    // Common XAMPP MySQL port (or 3306)
}

// Create connection with automatic environment detection
